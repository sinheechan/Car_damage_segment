...
if __name__ == "__main__":
    ...
    weights = 1.0 / weights
    weights = weights.cuda()  # weights => GPU
    weights = weights / weights.sum()

    parser = argparse.ArgumentParser()
    parser.add_argument('--train',  help='train') # 훈련
    parser.add_argument('--eval', help='evaluation') # 평가
    parser.add_argument('--task', help='damage vs part') # 수행할 작업
    parser.add_argument('--method', help='multi : multi models, single : single model') # 모델 선정 방법 
    parser.add_argument('--label', help="all, 2,3,4,5") # 라벨
    parser.add_argument('--cls', type = int, help = "n_class") # 클래스 수 지정
    parser.add_argument('--dataset', help = "val vs test") # 데이터셋 유형 지정
    parser.add_argument('--weight_file', help = "weight file name") # 가중치 파일 이름 지정
